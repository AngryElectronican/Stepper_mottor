...
uint16_t REGISTER[16]={0}; // registers
uint8_t memory_bytes[16]={0}; // bits
...
uint8_t ModRTU_get_memory_bits(uint8_t byte,uint8_t bit){
  return ((memory_bytes[byte]>>bit)&1);
}
void ModRTU_Init(){
...
uint8_t byte_count_mem=0;
uint8_t byte_count_out=0;
  for(uint8_t i=address;i<address+quantily;i++){
    byte_count_out=out_bit/8;
    byte_count_mem=i/8;
    if((memory_bytes[byte_count_mem]>>(i-byte_count_mem*8))&1){
      bits[byte_count_out]|=1<<(out_bit-8*byte_count_out);
    }else{
      bits[byte_count_out]&=~(1<<(out_bit-8*byte_count_out));
    }
    out_bit++;
...
void ModRTU_Write_Multiply_Bits(uint8_t* rx_mass){ //////////
  ...
  for(uint8_t i=address;i<quantily+address;i++){
    ...
    uint8_t byte_num_mem=i/8;
    if((bits_buf[byte_num]>>(bit_count-8*byte_num))&0x01){
        memory_bytes[byte_num_mem]|=1<<(i-8*byte_num_mem);
      }else{
        memory_bytes[byte_num_mem]&=~(1<<(i-8*byte_num_mem));
      }
      bit_count++;
